...
if [ ${ret} -eq 0 ]; then
    ...
    sudo docker run -tid  --name ${con}  -v /etc/localtime:/etc/localtime -v /etc/timezone:/etc/timezone -v ${now}:/opt/${prj} ${img} /bin/bash -c "ln -s /opt/docker/phantomjs-2.1.1-linux-x86_64 /opt/weather/phantomjs && mkdir /opt/weather/log && crontab /opt/weather/getdata.cron && service cron start && service cron reload && service cron status && tail -f /dev/null"
    #ret=$?
    #if [ ${ret} -eq 0 ]; then
